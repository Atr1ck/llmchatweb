...
第 2 步：在工作目录下创建 canvas-flow 根目录的 package.json，配置前端/后端开发脚本（server、web:dev、dev）以及 TypeScript/ts-node-dev 等基础依赖。
...
第 16 步：在 canvas-flow/README.md 中整理项目目录结构、安装依赖命令、启动命令及环境变量说明，便于直接运行与二次开发。
