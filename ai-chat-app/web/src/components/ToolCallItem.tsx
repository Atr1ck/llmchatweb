...
import { ChevronDown, ChevronRight, Loader2, Wrench, CheckCircle, XCircle, Clock, Cloud, Calculator, Search } from "lucide-react";
...
const toolMeta: Record<string, { label: string; icon: React.ReactNode }> = {
  get_weather: { label: "天气查询", icon: <Cloud className="h-3.5 w-3.5" /> },
  calculator: { label: "计算器", icon: <Calculator className="h-3.5 w-3.5" /> },
  web_search: { label: "网络搜索", icon: <Search className="h-3.5 w-3.5" /> },
};
...
function extractSummary(name: string, result: string): string | null {
  try {
    ...
    if (name === "get_weather" && parsed?.weather) {
      const parts = [parsed.city, parsed.weather];
      if (parsed.temp) parts.push(`${parsed.temp}°C`);
      if (parsed.humidity) parts.push(`湿度${parsed.humidity}%`);
      if (parsed.wind) parts.push(parsed.wind);
      return parts.filter(Boolean).join("，");
    }
    if (name === "calculator" && parsed?.result !== undefined) return `${parsed.expression} = ${parsed.result}`;
    if (name === "web_search" && parsed?.results) return `找到 ${parsed.totalResults ?? parsed.results.length} 条结果`;
