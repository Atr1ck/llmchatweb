import type { CreativeBrief, CreativeContextSnapshot, Message } from "../store/chatStore";
...
export async function getChatProviders(): Promise<ChatProviderOption[]> {
  const response = await fetch("/api/chat/providers");
  if (!response.ok) throw new Error(`读取模型列表失败（HTTP ${response.status}）`);
  const body = await response.json() as { providers?: ChatProviderOption[] };
...
export type SSECallbacks = {
  ...
  onCreativeContext: (context: CreativeContextSnapshot) => void;
  onCreativeBrief: (brief: CreativeBrief) => void;
  ...
  private dispatch(type: string, data: unknown) {
    ...
    switch (type) {
      ...
      case "creative_context":
        this.callbacks.onCreativeContext(data as CreativeContextSnapshot);
        break;

      case "creative_brief":
        if (d.brief) this.callbacks.onCreativeBrief(d.brief as CreativeBrief);
        break;
      ...
      case "error":
        this.callbacks.onError(typeof d.message === "string" ? d.message : "图片 Agent 执行失败");
        break;

      case "agent_warning":
        if (typeof d.message === "string") this.callbacks.onError(d.message);
        break;
    }
  }
...
export async function streamChat(
  ...
  options?: { signal?: AbortSignal; sessionId?: string; context?: CreativeAgentContext; providerId?: ChatProviderId }
): Promise<void> {
  const response = await fetch("/api/chat", {
    ...
    body: JSON.stringify({ messages, sessionId: options?.sessionId, context: options?.context, providerId: options?.providerId }),
    signal: options?.signal,
  });

  if (!response.ok) {
    throw new Error(`图片 Agent 请求失败（HTTP ${response.status}）`);
  }
